...
public class PessoaController {
    ...
    @ResponseBody
    public List<PessoaJuridica> buscaEmpresas(@PathVariable String categoria, @AuthenticationPrincipal UserImpl activeUser) {
        List<PessoaJuridica> empresas = this.pessoaService.buscaCategoria(categoria);

        if (activeUser != null) {
            User user = pessoaService.findById(activeUser.getUser().getId()).get();

            empresas = empresas.stream().filter(empresa -> {
                for (Endereco enderecoEmpresa : empresa.getEnderecos()) {
                    for (Endereco enderecoUsuario : activeUser.getUser().getEnderecos()) {
                        if (enderecoEmpresa.getCidade().equals(enderecoUsuario.getCidade())) {
                            return true;
                        }
                    }
                }
